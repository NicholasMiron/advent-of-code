  ...
  Object.entries(antennaLocations).forEach(([antenna, locations]) => {
    for (let i = 0; i < locations.length; i++) {
      for (let j = i + 1; j < locations.length; j++) {
        ...
        while((antinode1[0] >= 0 && antinode1[0] < mapWidth) && (antinode1[1] >= 0 && antinode1[1] < mapHeight) || (antinode2[0] >= 0 && antinode2[0] < mapWidth) && (antinode2[1] >= 0 && antinode2[1] < mapHeight)) {
          let xDiff = (x2 - x1) * mult;
          let yDiff = (y2 - y1) * mult;
